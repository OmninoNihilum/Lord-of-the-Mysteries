        ...
        if (!this.level().isClientSide()) {
            ...
            if (getRemoveAndHurt()) {
                if (!getSent() && this.getOwner() != null) {
                    ...
                    float sideMultiplier = Math.signum(getEndstoneStayAtY()); // -1 or 1
                    float sideDistance = Math.abs(getEndstoneStayAtX()); // Distance from player

                    // Calculate the target position
                    Vec3 sideOffset = rightDirection.scale(sideMultiplier * sideDistance);
                    Vec3 forwardOffset = horizontalLookDirection.scale(getEndstoneStayAtZ()); // Forward/backward offset

                    Vec3 targetPos = new Vec3(
                            this.getOwner().getX() + sideOffset.x + forwardOffset.x,
                            this.getOwner().getY() + getEndstoneStayAtY(), // Height offset
